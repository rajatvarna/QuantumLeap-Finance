...
const FeatureGate: React.FC<FeatureGateProps> = ({ children, requiredPlan }) => {
    ...
    const userLevel = user ? planLevels[user.plan] : -1; // Logged out users have a level below FREE
    ...
    return (
        <div className="text-center p-8 bg-background/50 m-4 rounded-lg flex items-center justify-center h-full">
            <div className="flex flex-col items-center">
                ...
                <h4 className="text-lg font-semibold text-text-primary">
                    {user ? 'Upgrade Required' : 'Login Required'}
                </h4>
                <p className="text-text-secondary mt-2">
                    This feature is available for <span className="font-bold text-accent">{requiredPlan}</span> plan subscribers.
                </p>
                <p className="text-sm text-text-tertiary mt-1">
                    {user ? 'Please upgrade your plan to access premium data.' : 'Please log in to access this feature.'}
                </p>
            </div>
        </div>
    );
